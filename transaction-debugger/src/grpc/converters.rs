...
impl ResponseConverter {
    ...
    pub fn to_grpc_response(dto: DebugResponseDto) -> DebugResponse {
        debug!(
            "Converting DTO to gRPC response, transaction_details: {:?}",
        ...
        let instruction_details: Vec<InstructionDetail> = dto
            .transaction_details
            .instruction_details
            .into_iter()
        ...
        let response = DebugResponse {
            ...
            transaction: Some(Self::convert_transaction_to_grpc(
                &dto.transaction,
                dto.slot,
                dto.block_time,
                &dto.meta,
        ...
        info!(
            "gRPC response created with transaction_details field: {}",
            response.transaction_details.is_some()
        );
        response
    }
                    ...
                    account_keys = keys
                        ...
                        .filter_map(|key| {
                            if let Some(pubkey) = key.get("pubkey").and_then(|p| p.as_str()) {
                                let signer =
                                    key.get("signer").and_then(|s| s.as_bool()).unwrap_or(false);
                                let writable = key
                                    .get("writable")
                ...
                if let Some(instrs) = message.get("instructions").and_then(|i| i.as_array()) {
                    instructions = instrs
                        ...
                        .filter_map(|instr| {
                                ...
                                let parsed = if let Some(parsed_obj) = instr.get("parsed") {
                                    Some(ParsedInstruction {
                                        r#type: parsed_obj
                                            ...
                                            .unwrap_or("unknown")
                                            .to_string(),
                                        info_json: serde_json::to_string(
                                            parsed_obj
                                                .get("info")
                            ...
                            } else if let Some(program_id) =
                                instr.get("programId").and_then(|p| p.as_str())
                            {
                                ...
                                let accounts = if let Some(acc_array) =
                                    instr.get("accounts").and_then(|a| a.as_array())
                                {
                                    acc_array
                                        .iter()
    ...
    fn convert_meta_to_grpc(meta: &solana_transaction_status::UiTransactionStatusMeta) -> Meta {
        ...
        let inner_instructions = match &meta.inner_instructions {
            OptionSerializer::Some(inner_instrs) => inner_instrs
                .iter()
                .map(|inner| {
                    let instructions = inner
                        ...
                        .map(|instr| {
                            ...
                            let (data, program_id, stack_height, parsed, program) = match instr {
                                solana_transaction_status::UiInstruction::Compiled(compiled) => (
                                    compiled.data.clone(),
                                    format!("program_{}", compiled.program_id_index),
                                    1u32,
                                    None,
                                    None,
                                ),
                                solana_transaction_status::UiInstruction::Parsed(parsed) => {
                                    let parsed_instr = Some(ParsedInstruction {
                                        ...
                                        info_json: serde_json::to_string(&parsed)
                                            .unwrap_or_default(),
                                    });
                                    (
                                        String::new(),
                                        "parsed_instruction".to_string(),
                                        1u32,
                                        parsed_instr,
                                        None,
                                    )
